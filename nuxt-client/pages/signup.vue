...
const signup = async () => {
    if (loading.value || !(body.value.email && body.value.password && body.value.identifier)) {
        return;
    }
    ...
    const { data, errors } = await fetch("https://prod.tradify.dk/auth/provider/email/signup", {
        method: "POST",
        headers: {
